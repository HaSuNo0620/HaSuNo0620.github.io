...
marker heads must follow light/dark theme colors, and known layout corrections
are applied to generated assets before Astro builds the site.
"""
...
def theme_marker_heads(svg: str) -> str:
    ...
    if f'fill="{LIGHT_ACCENT}"' in svg and '<marker' in svg:
        ...
        changed = True
    if changed and ".arrowhead-ink" not in svg:
        svg = svg.replace("</svg>", ARROW_THEME_STYLE + "</svg>")
    elif changed and ARROW_THEME_STYLE.strip() not in svg:
        # The generated figure style does not otherwise define arrowhead classes.
        svg = svg.replace("</svg>", ARROW_THEME_STYLE + "</svg>")
    return svg


def fix_known_layouts(path: Path, svg: str) -> str:
    """Apply deterministic layout fixes that depend on the generated asset."""
...
def normalize_fixed_black(svg: str) -> str:
    ...
    svg = re.sub(r'(?i)(fill|stroke)="(?:#000000|#000|black)"', rf'\1="{LIGHT_INK}"', svg)
    return svg
