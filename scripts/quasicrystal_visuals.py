...
.guide{{stroke:{DARK['line']}}}.axis{{stroke:{DARK['ink']}}}
...
def order_comparison() -> None:
    ...
    rows = [
        (90, "周期", "AB" * 9, "accent"),
        (220, "規則的な非周期", "LSLLSLSLLSLLSLSLLS", "green"),
...
def quasiperiodic_signal_phase() -> None:
    ...
    p.append('<line class="mutedline" x1="462" y1="77" x2="505" y2="77"/><text class="small" x="518" y="82">周波数比 1:2</text>')
    p.append('<line class="primary" x1="462" y1="101" x2="505" y2="101"/><text class="small" x="518" y="106">周波数比 1:√2</text>')
    p.append('<text class="math" x="390" y="229" text-anchor="middle">x / π</text>')

    for left, alpha, title, cls in [
        (100, 2.0, "有理比：軌道が閉じる", "mutedline"),
        (430, math.sqrt(2), "無理数比：軌道が閉じない", "primary"),
    ]:
        top = 275
        size = 190
...
def cut_and_project() -> None:
    ...
    for s in (-strip, strip):
        x1 = cx + scale * (-7 * ux + s * vx)
        y1 = cy - scale * (-7 * uy + s * vy)
        x2 = cx + scale * (7 * ux + s * vx)
        y2 = cy - scale * (7 * uy + s * vy)
    ...
    p.append('<text class="small" x="520" y="118">射影先の方向 E∥</text>')
    p.append('<text class="small" x="252" y="238">点を選別する方向 E⊥</text>')
    p.append('<text class="label" x="36" y="54">帯の中の格子点だけを選んで射影する</text>')
...
def diffraction_fourier_module() -> None:
    p: list[str] = []
    panels = [(55, "周期：基本波数 1個"), (405, "準周期：独立基本波数 2個")]
