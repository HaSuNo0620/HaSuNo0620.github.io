...
function detectCycle(start: string, scenes: Record<string, StoryScene>): boolean {
  ...
  const visit = (id: string): boolean => {
    ...
    visiting.add(id);
    for (const next of targets(scenes[id])) if (visit(next)) return true;
    visiting.delete(id);
    visited.add(id);
    return false;
  };
...
export function validatePack(pack: QualificationPack): ValidationReport {
  ...
  for (const section of pack.manifest.sections) {
    if (sectionIds.has(section.id)) error('DUPLICATE_SECTION_ID', `Duplicate section id: ${section.id}`, 'manifest.sections');
  ...
  for (const node of pack.knowledge) {
    if (knowledgeIds.has(node.id)) error('DUPLICATE_KNOWLEDGE_ID', `Duplicate knowledge id: ${node.id}`, `knowledge.${node.id}`);
    knowledgeIds.add(node.id);
    coverage.set(node.id, { knowledgeId: node.id, encounter: 0, apply: 0, discriminate: 0, recall: 0 });
  }
  ...
  for (const story of pack.stories) {
    ...
    for (const variant of story.variants) {
      const prefix = `stories.${story.id}.${variant.id}`;
      const sceneIds = new Set(Object.keys(variant.scenes));
      if (!sceneIds.has(variant.startScene)) {
        error('INVALID_START_SCENE', `${story.id}/${variant.id} start scene ${variant.startScene} does not exist`, `${prefix}.startScene`);
        continue;
      }
          ...
          for (const choice of scene.choices) {
            for (const signal of choice.learning) {
              if (!knowledgeIds.has(signal.knowledgeId)) {
                error('UNKNOWN_KNOWLEDGE_ID', `${story.id}/${variant.id}/${sceneId}/${choice.id} references unknown knowledge ${signal.knowledgeId}`, `${prefix}.scenes.${sceneId}.${choice.id}.learning`);
              } else {
                coverage.get(signal.knowledgeId)![signal.mode] += 1;
              }
            }
      ...
      if (defaultCount !== 1) error('INVALID_DEFAULT_ENDING_COUNT', `${story.id}/${variant.id} must have exactly one default ending`, `${prefix}.endings`);
  ...
  for (const row of coverageRows) {
    const total = row.encounter + row.apply + row.discriminate + row.recall;
    if (total === 0) warning('NO_STORY_COVERAGE', `${row.knowledgeId} has no story coverage`, `knowledge.${row.knowledgeId}`);
    else if (row.apply === 0 && row.discriminate === 0) warning('ENCOUNTER_ONLY_COVERAGE', `${row.knowledgeId} is never applied or discriminated`, `knowledge.${row.knowledgeId}`);
  }
