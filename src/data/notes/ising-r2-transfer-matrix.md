...
この意味で R=2 で起きているのは、correlation strength の単純な増減ではなく、**どの空間モードが長距離記憶を運ぶか**という問いの変化である。
...
R=1 では単一指数相関を Fourier 変換するだけで $\chi(q)$ が閉じた。R=2 では

$$
C(r)\sim e^{-r/\xi}\cos(q_*r+\phi)
$$

なので、感受率は自然に有限波数 $q\simeq q_*$ の構造を持つはずである。

ただし次の段階では漸近形だけを Fourier 変換するのではなく、**転送行列から有限距離の $C(r)$ を正確に構成し、その全体を Fourier 変換して $\chi(q)$ を求める**。これにより「固有値の位相 $q_*$」と「実際の応答ピーク位置」がどこまで一致するかを確認できる。
