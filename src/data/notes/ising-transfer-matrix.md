...
外場が格子上で周期 $L$ を持ち $h_{i+L}=h_i$ なら、1周期分の積

$$
M_L=T_1T_2\cdots T_L
$$

を導入できる。$N$ が $L$ の整数倍なら $Z=\operatorname{Tr}M_L^{N/L}$ であり、$M_L$ の最大固有値を $\Lambda_+$ とすれば、
...
したがって $R=1$ は単に「解ける模型」ではなく、**相互作用範囲、局所記憶、欠陥、相関長、波数応答が最も単純な形で接続する基準点**として使える。
