---
title: "1次元一様有限範囲 cosine-Z2 スピン系 — 高次壁相互作用と有限履歴"
summary: "有限範囲の1次元Ising鎖を一般化し、ドメイン壁表示では高次の局所相互作用、壁列では有限ステップの相関が現れることを整理する。第三近接までを最初の具体例として読む。"
...
この一般化で増えるのは 転送行列 のサイズだけではない。相互作用範囲 $N$ は、ドメイン壁表示では**最大 $N$ 体の局所相互作用**へ、条件付き統計では**最大 $N-1$ step の空間相関**へ写る。

![相互作用範囲と壁相互作用・相関長の対応](/figures/ising-rn/range-memory-map.svg)
...
## 2. スピン表示の state size と ドメイン壁列の 相関長 は別の数え方になる
...
N\text{ range}
\Longrightarrow
(N-1)\text{ ステップのドメイン壁相関}}
$$

となる。

ここでいう相関は時間発展の非平衡相関ではなく、**鎖に沿って次の局所確率を決めるために必要な履歴**である。

## 3. 第三近接では3体ドメイン壁相互作用と2-step 履歴が同時に現れる
...
## 7. 有限範囲は局所相互作用と有限履歴の二つの言葉で同じものを表す
...
\text{ドメイン壁過程: 最大 }N-1\text{ step 履歴}\\
