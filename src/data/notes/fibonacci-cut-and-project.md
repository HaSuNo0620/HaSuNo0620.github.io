...
ここで黄金比が急に現れるのは少し不思議なので、置換則そのものから確認しておく。

1回の置換で $L$ は $LS$ を生み、$S$ は $L$ を生む。したがって個数ベクトルは
...
つまり黄金比は後から付けた数字ではなく、**置換規則の成長率そのもの**としてすでに入っている。

ただ、置換則だけでは「なぜ高次元格子とつながるのか」は見えにくい。そこで同じ列を幾何学として作り直す。
...
最終的に原子位置として使うのは $x_{\parallel}$ なので、$E_{\parallel}$ を **physical space** と呼ぶ。

一方、$x_{\perp}$ はその格子点を採用するかどうかの判定に使う。こちらが **internal space** である。
...
式だけ見ると少し硬いが、図では単に「斜めの帯に入った格子点だけ拾う」と読める。
...
ここで $L,S$ の意味が具体的になる。置換則の記号だったものが、**2次元格子の基本ステップを1次元へ射影した長さ**として見える。

置換側では $L,S$ の個数比に、射影側では長さの比と無理数傾きに、同じ $\tau$ が現れる。Fibonacci chain の代数と幾何が同じ黄金比で結ばれている。
