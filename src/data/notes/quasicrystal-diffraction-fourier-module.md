...
周期結晶では Bragg peak と reciprocal lattice がほとんど同時に現れるので、「Bragg peak があるなら周期格子もある」と思いやすい。ただ、Bragg peak が直接測っているものを分けて考えると、その含意は少し違う。
...
## 1. Bragg peak が見ているのは長距離の位相コヒーレンス
...
この区別だけで、準結晶でも Bragg peak が存在できる余地が生まれる。

## 2. 周期結晶では reciprocal lattice が peak 位置を固定する
...
実空間の周期 $a$ に対して、波数空間には基本間隔 $2\pi/a$ の格子ができる。これが reciprocal lattice である。
...
## 3. 準周期構造では rank が物理次元を超える

[準周期関数のノート](/notes/quasiperiodic-functions-torus/)で出てきたように、1次元でも複数の独立な基本周波数を持てる。
...
ただし peak の位置が無秩序なわけではなく、二つの整数 $(m,n)$ で index できる。

このとき、peak 集合を生成するために必要な独立な基本波数の個数を rank と呼ぶ。
...
「逆空間に規則がない」のではなく、**逆空間の整数構造が物理空間の次元より高い**と見る方が近い。

## 4. 高次元 reciprocal lattice の射影として読む

cut-and-project では、高次元格子点を physical space と internal space の二方向へ分解した。

波数空間でも同じ分解ができる。

高次元側の reciprocal lattice にある波数ベクトルを $\mathbf K$ とし、
...
実空間の cut-and-project と逆空間の Fourier module は、同じ高次元幾何学の表裏になっている。

## 5. peak 候補が稠密でもスペクトルは連続とは限らない
...
候補位置がどこにでもあるなら、回折像は連続的に埋まりそうにも見える。ただ、**peak の位置が許されることと、その peak が強いことは別**である。
...
さらに、位置集合が密であることと連続スペクトルであることも別である。理想的な Fibonacci chain では、回折は個々の鋭い Bragg peak からなる pure-point 型だが、その peak の位置集合自体は稠密になりうる。
...
「位置は高密度でも、強度は強く階層化され、各成分自体は鋭い」という像になる。
...
準結晶の秩序は「実空間の単位胞」に保存されているのではなく、**有限 rank の波数構造と、それを生む高次元幾何学**に保存されていると読むとまとまりがよい。
...
実空間では周期を失っていても、構造を生成する整数的な秩序まで失われているわけではない。ここから inflation、periodic approximant、phason、Penrose tiling など、準結晶固有の自由度へつながる。
